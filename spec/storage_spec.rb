...
describe 'Magma::Storage' do
    ...
    it 'returns a copy URL.' do
        ...
        if Magma.instance.storage.instance_of? Magma::Storage::Metis
            ...
            expect(copy_url.include? 'X-Etna-Authorization').to be true
            expect(copy_url.include? 'X-Etna-Expiration').to be true
            expect(copy_url.include? 'X-Etna-Nonce').to be true
            expect(copy_url.include? 'X-Etna-Id').to be true
            expect(copy_url.include? 'X-Etna-Headers').to be true

            # Just to make sure we're sane
            expect(copy_url.include? 'upload/magma').to be false
            expect(copy_url.include? 'download/magma').to be false
        end
    end
