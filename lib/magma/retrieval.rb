  ...
  class Retrieval
    ...
    class StringFilter
      ...
      def filter_term term, attributes
        ...
        if match.nil?
          raise ArgumentError, "Filter term '#{term}' does not parse"
        end

        att = attributes.find{|a| a.name == att_name.to_sym}
        unless att.is_a?(Magma::Attribute)
          raise ArgumentError, "#{att_name} is not an attribute"
        end
      ...
      def string_op(operator)
        ...
        when '~'
          ...
          raise(
            ArgumentError,
            "Invalid operator #{operator} for string attribute!"
          )
        end
      end
      ...
      def boolean_op(operator, value)
        unless operator == '='
          raise(
            ArgumentError,
            "Invalid operator #{operator} for boolean column!"
          )
        end
